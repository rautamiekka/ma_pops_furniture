...
local function handle_rightclick(pos, node, sound_name, new_node_name)
    ...
    -- store as string (meta stores strings)
    meta:set_string("hwnd", tostring(new_handle))
end
...
local function register_tv_node(name, desc, mesh, tiles, sound, new_node_name, light, groups, additional_props)
    local node_def = {
        ...
        selection_box = { type = "fixed", fixed = { -0.45, -0.5, -0.5, 0.45, 0.4, 0.45 } },
        collision_box = { type = "fixed", fixed = { -0.45, -0.5, -0.5, 0.45, 0.4, 0.45 } },
...
register_tv_node(
    ...
    {
        { name = tex("default_tree.png") },
        -- black glass overlay (wrap core textures via tex)
        { name = tex("wool_black.png") .. "^" .. tex("default_glass_detail.png") .. "^[colorize:black:225" },
    },
    "mp_off",
    "ma_pops_furniture:tv_rainbow",
    1,
...
register_tv_node(
    ...
    common_groups,
    {
        ...
        selection_box = { type = "fixed", fixed = { -0.98, -0.5, -0.10, 0.98, 0.8, 0.10 } },
        collision_box = { type = "fixed", fixed = { -0.98, -0.5, -0.10, 0.98, 0.8, 0.10 } },
    }
)
